...
void cuda_init_random_state(const uint size) {
    // malloc
    CUDA_CHECK(hipMalloc((void**) &devStates, size * sizeof(hiprandState)));

    dim3 block((size-1)/MAX_THREAD_PER_BLOCK + 1, 1, 1);
    dim3 thread_in_block(MAX_THREAD_PER_BLOCK, 1, 1);

    // kernel
    hipLaunchKernelGGL(cuda_init_rand_kernel,block,thread_in_block,0,0,devStates);
...
__global__
void cuda_Variable_glorot_kernel(float *data, hiprandState *state, int size, float scale) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id < size)
